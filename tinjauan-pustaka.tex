...
(RGB-D); dan (iv) integrasi YOLO dengan data RGB-D. Sebanyak lebih dari 200 karya
...
pengecualian untuk karya fondasi). Total lebih dari 200 referensi disaring dan
...
sapkota2024yoloagri,sapkota2025yologenesis,vijayakumar2024yolo,alif2024yoloevolution,
sohan2024yolov8review}. Varian mutakhir memperluas lintasan ini: Gold-YOLO
memperkenalkan mekanisme pengumpulan-dan-distribusi untuk fusi fitur
multi-skala~\cite{wang2023goldyolo}, YOLO-World menghadirkan deteksi
kosakata-terbuka berbasis penyelarasan visi--bahasa~\cite{cheng2024yoloworld},
dan YOLO26 mengusung detektor waktu-nyata ujung-ke-ujung generasi
2025~\cite{sapkota2025yolo26}. Ringkasan evolusi disajikan pada Tabel~\ref{tab:yolo}.
...
DETR untuk konvergensi lebih cepat~\cite{zhu2021deformabledetr}. Rangkaian
penyempurnaan DETR menyusul: Conditional DETR mempercepat konvergensi melalui
kueri spasial terkondisi~\cite{meng2021conditionaldetr}, DN-DETR menstabilkan
pelatihan lewat \emph{query denoising}~\cite{li2022dndetr}, DINO memadukan
\emph{denoising} dan seleksi kueri campuran untuk akurasi
puncak~\cite{zhang2023dino}, sedangkan Co-DETR memperkuat pelatihan melalui
penugasan label kolaboratif~\cite{zong2023codetr}. Sparse R-CNN menawarkan
alternatif berbasis himpunan proposal terpelajari berjumlah
tetap~\cite{sun2021sparsercnn}. Untuk inferensi waktu-nyata, RT-DETR menjadi
detektor transformer yang menyaingi YOLO~\cite{zhao2024rtdetr}, diikuti RF-DETR
hasil \emph{neural architecture search}~\cite{robinson2025rfdetr} dan Le-DETR
dengan enkoder ringan~\cite{huang2026ledetr}. \emph{Vision
Transformer}~\cite{dosovitskiy2021vit} dan \emph{Swin Transformer} dengan jendela
bergeser~\cite{liu2021swin} menjadi \emph{backbone} tugas prediksi padat, dengan
Pyramid Vision Transformer~\cite{wang2021pvt}, Swin Transformer V2 yang
menskalakan kapasitas dan resolusi~\cite{liu2022swinv2}, serta ConvNeXt yang
memodernkan CNN murni agar setara transformer~\cite{liu2022convnext} sebagai
kelanjutannya. \emph{Backbone} residual~\cite{he2016resnet} dan modul atensi
...
Estimasi \emph{metrik} berskala nyata ditangani BTS melalui panduan bidang planar
lokal~\cite{lee2019bts}, NeWCRFs via CRF berjendela ternural~\cite{yuan2022newcrfs},
ZoeDepth yang menjembatani kedalaman relatif dan metrik~\cite{bhat2023zoedepth},
serta Metric3D yang menyeragamkan skala lintas kamera~\cite{yin2023metric3d}. Depth
Anything V2 mempertajam detail dan ketahanan~\cite{yang2024depthanythingv2},
sedangkan Marigold memanfaatkan \emph{diffusion} untuk estimasi kedalaman
generatif~\cite{ke2024marigold}. Perkembangan 2025--2026 menandai model fondasi
geometri Depth Anything 3~\cite{lin2025depthanything3}, kedalaman metrik universal
lintas-kamera UniDAC~\cite{ganesan2026unidac}, estimasi waktu-nyata bermemori
spasial AsyncMDE~\cite{ma2026asyncmde}, serta Focusable Monocular Depth
Estimation~\cite{du2026focusabledepth}; survei terbaru merangkum tren estimasi
kedalaman metrik monokular ini~\cite{zhang2025metricdepthsurvey}.
...
kedalaman hierarkis~\cite{wu2023hidanet}. Arah lain menekankan kolaborasi dan
efisiensi: CoNet mengkolaborasikan isyarat tepi, kedalaman, dan
saliency~\cite{ji2020conet}, DCF mengalibrasi lalu memfusikan
kedalaman~\cite{ji2021dcf}, SPNet memakai jaringan pemurni kedalaman
selektif~\cite{zhou2021spnet}, CAVER membangun fusi lintas-modal berbasis
transformer sederhana~\cite{pang2023caver}, dan MobileSal menargetkan inferensi
ringan pada perangkat bergerak~\cite{wu2022mobilesal}. Karya termutakhir GL-DMNet
mengusung pembelajaran mutual ganda global--lokal~\cite{yi2025gldmnet}. Survei Zhou
...
pembelajaran representasi RGB-D untuk segmentasi~\cite{yin2024dformer}. SegFormer
menyederhanakan enkoder transformer hierarkis tanpa \emph{positional
encoding}~\cite{xie2021segformer}, EMSANet menyatukan segmentasi semantik dan
instans secara efisien~\cite{seichter2022emsanet}, GeminiFusion menerapkan fusi
lintas-modal per-piksel yang terkontrol~\cite{jia2024geminifusion}, dan Omnivore
menyatukan klasifikasi citra, video, serta RGB-D dalam satu
model~\cite{girdhar2022omnivore}. Pendekatan terbaru DiffPixelFormer
mengeksplorasi fusi tingkat-piksel berbasis \emph{diffusion} untuk \emph{scene}
dalam-ruangan~\cite{gong2025diffpixelformer}. Dataset
...
Pendekatan berbasis koordinat padat dan geometri turut memperkaya bidang ini:
GDR-Net meregresi pose secara terpandu-geometri~\cite{wang2021gdrnet}, ZebraPose
mengodekan permukaan objek secara hierarkis-diskrit~\cite{su2022zebrapose}, dan
OnePose memungkinkan estimasi pose objek baru tanpa model CAD~\cite{sun2022onepose}.
...
besar~\cite{fang2020graspnet}. Sintesis \emph{grasp} 6-DoF diperluas oleh
Contact-GraspNet yang memprediksi grasp langsung dari awan titik
adegan~\cite{sundermeyer2021contactgraspnet} dan VGN yang menghasilkan grasp
volumetrik waktu-nyata dari TSDF~\cite{breyer2021vgn}.
...
~\cite{wang2019pseudolidar,you2020pseudolidarpp}. Deteksi berbasis pusat objek
CenterPoint menyederhanakan alur pada awan titik~\cite{yin2021centerpoint}, PV-RCNN
memadukan fitur titik dan voxel untuk proposal berkualitas~\cite{shi2020pvrcnn},
sedangkan pendekatan kamera-saja berbasis transformer BEVFormer~\cite{li2022bevformer}
dan DETR3D~\cite{wang2022detr3d} memprediksi objek 3D langsung dalam ruang
\emph{bird's-eye view}. Dataset
...
pembelajaran lintas-modal terpandu-ketidakpastian~\cite{kim2022cmpd}, serta
\emph{cross-modality fusion transformer} yang memfusikan RGB dan termal di tingkat
fitur via atensi~\cite{fang2022cft} meningkatkan
...
RGB-D~\cite{murartal2017orbslam2}, yang diperluas ORB-SLAM3 dengan dukungan
visual-inersial dan multi-peta~\cite{campos2021orbslam3}. Lingkungan dinamis ditangani dengan menggabungkan
...
diseimbangkan~\cite{soares2019visualslam}. Pendekatan pembelajaran mendalam
DROID-SLAM mencapai estimasi pose dan kedalaman padat yang akurat melalui
pembaruan iteratif berbasis \emph{bundle adjustment}~\cite{teed2021droidslam}.
...
tinggi~\cite{zhang2019remotesensing}. Pendekatan transformer terbaru RiO-DETR
menghadirkan deteksi objek berorientasi waktu-nyata untuk citra
udara~\cite{hu2026riodetr}.
...
khanam2024yolov11,long2020ppyolo,wang2023goldyolo,cheng2024yoloworld,
sapkota2025yolo26} \\
...
liu2021swin,he2016resnet,woo2018cbam,meng2021conditionaldetr,li2022dndetr,
zhang2023dino,zong2023codetr,sun2021sparsercnn,zhao2024rtdetr,robinson2025rfdetr,
huang2026ledetr,wang2021pvt,liu2022swinv2,liu2022convnext} \\
...
ji2021monoindoor,ming2021depthsurvey,lee2019bts,yuan2022newcrfs,bhat2023zoedepth,
yin2023metric3d,yang2024depthanythingv2,ke2024marigold,lin2025depthanything3,
ganesan2026unidac,ma2026asyncmde,du2026focusabledepth,zhang2025metricdepthsurvey} \\
...
wu2023hidanet,zhou2021rgbdsurvey,ji2020conet,ji2021dcf,zhou2021spnet,
pang2023caver,wu2022mobilesal,yi2025gldmnet} \\
...
zhang2023cmx,zhou2022pgdenet,yin2024dformer,xie2021segformer,seichter2022emsanet,
jia2024geminifusion,girdhar2022omnivore,gong2025diffpixelformer} \\
...
depierre2018jacquard,wang2021gdrnet,su2022zebrapose,sun2022onepose,
sundermeyer2021contactgraspnet,breyer2021vgn} \\
...
\section{Kesimpulan}
% =====================================================================
Tinjauan atas lebih dari 200 karya (2019--2026, dengan konteks fondasi) memetakan
