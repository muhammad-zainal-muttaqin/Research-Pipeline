...
[`../docs/eksperimen/EKSPERIMEN.md`](../docs/eksperimen/EKSPERIMEN.md); **SR**
merujuk laporan solusi di [`../docs/eksperimen/SR/`](../docs/eksperimen/SR/).
...
**Jangan pakai `pipeline/prepare_depth.py` untuk data Orbbec** — ia berasumsi
depth sudah tersejajar ke RGB, dan untuk dataset ini asumsi itu salah (meleset
median 29 px). Lihat [SR-015](../docs/eksperimen/SR/SR-015-depth-sensor-4kanal.md).

## Folder hasil

| Folder | Isi |
|---|---|
| [`results/`](results/) | JSON mentah, dikelompokkan per eksperimen — lihat [`results/README.md`](results/README.md) |
| `runs/` | Konfigurasi + kurva per-epoch tiap run (`args.yaml`, `results.csv`, `metrics.csv`, `training_config.json`) |
| `logs/` | Keluaran konsol, sudah dibersihkan dari progress-bar |
| `figures/` | Confusion matrix, kurva PR, kurva F1-confidence (E-021) |
| `splits_rgb/` | Definisi split per pohon 716/96/141, irisan nol |
| `splits_depth/` | Split per pohon 245/35/72 untuk SawitMVC-Depth (E-022), irisan nol |
