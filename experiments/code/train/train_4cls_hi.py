...
m = YOLO(a.weights)
m.train(data=str(REPO_ROOT / "reproduce" / "experiments" / "config" / "data_rgb.yaml"), epochs=a.epochs, imgsz=a.imgsz, batch=a.batch,
...
r = m.val(data=str(REPO_ROOT / "reproduce" / "experiments" / "config" / "data_rgb.yaml"), split="val", imgsz=a.imgsz, batch=4,
