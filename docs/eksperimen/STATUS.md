...
**Terakhir diperbarui:** 2026-07-30 · **Status:** aktif — RF-DETR-L (E-021) jadi detektor terbaik baru dan **sasaran mAP50 0,60 pada test terlewati** (0,6038); perbandingan param-adil 1-protokol sudah tuntas. Yang tersisa: mAP50-95 (−0,023).

**Baru (2026-07-29/30) — dataset `SawitMVC-Depth` (depth SENSOR Orbbec) sudah
diuji: E-022 / [SR-015](SR/SR-015-depth-sensor-4kanal.md).** Fusi 4-kanal awal
**DIPALSUKAN** (satu-satunya delta signifikan justru dari kanal DERAU), tetapi
kandungan informasi depth **terkonfirmasi pada model besar** (RT-DETR-L: B4
+0,1001 CI [+0,0062; +0,1618] vs kontrol derau). Arah lanjutan: **E-023 fusi
menengah**.
...
| **Depth SENSOR 4-kanal awal tidak menaikkan mAP** — kanal DERAU justru satu-satunya delta signifikan (+0,0437) | fusi awal ditutup; arah = fusi menengah | **SR-015**, E-022 |
...
- ~~**Depth sensor Gemini** — `pipeline/` menunggu data fisik.~~ **SUDAH DIUJI
  (2026-07-29, E-022/SR-015)** pada `ULM-DS-Lab/SawitMVC-Depth` (352 pohon, 1.408
  citra RGB-D, sensor Orbbec). Lubang ini tertutup. Keputusan yang perlu diambil:
  lanjut ke **E-023 fusi menengah** atau menambah data B4 lebih dulu (B4 hanya 148
  kotak di seluruh dataset baru).
- **`pipeline/prepare_depth.py` TIDAK boleh dipakai untuk data Orbbec.** Ia
  ...
  `experiments/build/reproject_depth.py`. Rentang `fourch.Z_NEAR/Z_FAR` (0,3–8,0 m)
  juga tidak cocok untuk sensor ini — dipakai 0,8/15,0 m.
---
