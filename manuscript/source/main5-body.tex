...
% Claims are restricted to PDFs represented in audit/evidence-matrix-182.csv.
...
Unlike previous reviews on oil-palm or fruit detection, which organize evidence by sensor, ripeness, or detector family, this review makes the cross-view identity of each physical fruit the central design variable. Its contributions are threefold: (1)~a six-mechanism taxonomy (M0--M5) that classifies how existing systems resolve, or fail to resolve, duplicate observations; (2)~a reproducible search trace covering 21{,}066 records from Scopus and OpenAlex, with 44 verified full-text studies coded against the taxonomy; and (3)~a falsifiable experimental agenda that specifies the dataset, metric hierarchy, and ablation conditions required to evaluate depth-aided, duplicate-aware fruit inventory under realistic field constraints. Table~\ref{tab:position} makes the positioning explicit.
...
Measurements produced by the authors themselves are governed by a separate evidence contract, stated in the companion results report, and are never counted as corpus evidence here.
...
\section{Depth estimation, fusion, and three-dimensional perception}
...
\section{From detection to inventory: association, evaluation, and agricultural evidence}
...
The verified corpus and the focused search together yield a substantially wider agricultural evidence base than was available to the 182-source corpus alone, covering oil-palm detection and classification, multi-view fruit counting, temporal tracking in orchards, and geometric reconstruction pipelines. This section separates oil-palm systems from other tree-fruit systems and positions the review against previous reviews on similar targets.
...
\section{Synthesis, challenges, and research directions}

\subsection{Evidence-map synthesis}
...
\subsection{A falsifiable research agenda for RGB-D TBS perception}
\label{sec:agenda}
...
The immediate contribution of the evidence map is a testable program rather than a claimed final architecture; the conceptual pipeline it points to, with each stage corresponding to one ablation below, is sketched in Figure~\ref{fig:pipeline}. The agenda is stated here in the form in which it was written, before any of it had been run, so that its wording is not fitted to any later outcome. First, construct a paired RGB-D FFB dataset with calibration metadata and annotations for bunch instances, maturity, occlusion, apparent size, illumination, depth validity, and where possible cross-frame identity. Reference counts must be collected at an operational unit such as a tree, short row, or traversal, because frames alone cannot validate a counting system, and splits must separate trees, sequences, and sessions to prevent optimistic transfer.
...
Parts of this agenda have since been executed by the authors on public oil-palm data and are reported, with their audit trail, gate register, and negative results, in the companion evidence-audited results report. That report is a separate document with a separate evidence contract; its measurements are not literature and are deliberately kept out of the evidence map so that no internal number can be read as corpus evidence.
...
Reliable fruit-inventory perception should be framed as detection, class assignment, geometry, cross-view association, and final counting in one pipeline. Traced across a decade of history, YOLO is a well-motivated real-time RGB baseline; monocular and sensor depth is a conditional geometric cue whose meaning must be declared; and RGB-D fusion is a hypothesis to be evaluated under realistic sensor and illumination failures. The 182-source verified evidence map makes these boundaries inspectable and shows both how the field reached its current tools and where its evidence stops.

The central research implication is unchanged: progress requires a calibrated paired RGB-D FFB benchmark and an evaluation protocol that measures detection, class assignment, geometry, duplicate suppression, count error, and latency together. The search protocol produced 21,066 resolved records; 20,035 advanced after abstract screening; 250 were shortlisted for targeted review; and 44 full-text studies supplied the current focused evidence matrix. The direct evidence includes oil-palm detector and dataset studies, multi-angle and RGB-depth systems, fruit SfM and NeRF reconstruction, LiDAR--camera fusion, and orchard video counting. Together they support the six-mechanism design space spanning intra-view counting, statistical correction, appearance matching, temporal tracking, geometric association, and learned multi-view association. The authors' own attempts against that protocol, including their negative and withdrawn results, are reported in the companion results document.
