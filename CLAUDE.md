...
> **Melanjutkan eksperimen?** Baca **[`experiments/STATUS.md`](experiments/STATUS.md)** dulu —
...
> 1-protokol tuntas, lihat [SR-014](experiments/SR/SR-014-rfdetr-dinov2.md)).
...
`manuscript/guides/PANDUAN-PENULISAN.md`). Jangan beralih ke Inggris kecuali diminta.
...
| Angka | Nilai |
|---|---|
| Entri di ledger (`manuscript/source/references.bib`) | 202 |
| Entri terverifikasi (ada PDF lokal) = korpus naskah | **182** (`literature/entries/`) |
| Entri ditahan (PDF sumber tak tersedia) | 20 (`literature/withheld/`) |
| Klaster tema (taksonomi naskah `literature/synthesis.md`) | 14 |
| Label tema (segmen terakhir nama berkas `literature/entries/`) | 17 |
| Rentang fokus | 2019–2026 (+ fondasi 2012–2018) |

**14 dan 17 bukan kontradiksi** dan sudah direkonsiliasi saat audit
(`manuscript/guides/PLAN-SITUS.md` §"Catatan rekonsiliasi sumber", `manuscript/figures/C02-distribusi-tema.md` §2):
...
Angka 182 itu **invarian yang dijaga**: naskah, situs, `literature/synthesis.md`, dan
`audit/claim-audit-182.md` semuanya diselaraskan ke angka ini. Jika mengubah jumlah
...
| Berkas / folder | Isi |
|---|---|
| `manuscript/source/evidence-body.tex` | **Isi naskah aktif.** Semua penyuntingan naskah masuk ke sini. |
| `manuscript/source/main.tex` / `manuscript/source/main-elsarticle.tex` | Driver IEEEtran / Elsevier; keduanya `\input` `manuscript/source/evidence-body.tex`. |
| `legacy/tinjauan-pustaka.tex` | Draf lama mandiri — **tidak dipakai**; jangan disunting tanpa diminta. |
| `literature/entries/` | 182 berkas ringkasan (satu makalah = satu berkas) + `INDEX.md` (urut nomor) + `INDEX-TAHUN.md` (per tahun & tema). |
| `literature/withheld/` | 20 entri ditahan; jangan dimasukkan ke naskah. |
| `manuscript/source/references.bib` | 202 record BibTeX. |
| `literature/synthesis.md` | Sintesis lintas makalah. |
| `manuscript/figures/` | Figur final F01–F08 (`-en.jpg`), C01, C02, plus brief `.md`-nya. |
| `experiments/results/` | Hasil JSON/NPZ per eksperimen (E-001…F-007), log training, konfigurasi run, split dataset. |
| `experiments/` | `STATUS.md` (baca pertama), `EKSPERIMEN.md` (log append-only), `METRICS.md` (tabel definitif), `LAPORAN-EKSPERIMEN.md` (tayang di situs), `SR/` (laporan per-ide). |
| `manuscript/guides/` | Panduan penulisan, rencana naskah & situs, keputusan reframe, prompt figur. |
| `audit/` | Audit pra-submisi, claim audit 182, register klaim, matriks bukti. |
| `literature/references/` | Bahan luar: PDF baseline SawitMVC, deep research report, revisi dosen. |
| `legacy/` | Draf & figur usang. |
...
Diambil dari `manuscript/guides/PANDUAN-PENULISAN.md` §2 — baca lengkap sebelum menulis entri.
...
1. Sidecar `"alignedTo": "color"` **MENYESATKAN** — buffer masih di grid kamera
   ...
   **Sudah diselesaikan:** depth dataset ini diproses lewat
   `experiments/code/build/reproject_depth.py` (reproyeksi penuh), dan seluruh angka E-022
   memakai jalur itu. `pipeline/prepare_depth.py` bukan alternatif yang salah pakai —
...
`literature/references/SawitMVC-DiB-2026.pdf`
(teks terekstrak: `experiments/code/refs/dib-text.txt`). Angka di bawah sudah
...
- **`class_mismatch`**: flag otomatis saat kelas yang dianotasi berbeda antar-sisi
  ...
  `experiments/code/analysis/class_mismatch_stats.py` (21 Juli 2026): 0 ketidaksepakatan dari
...
(temuan sapuan 28 titik fusi Ophoff dkk., `manuscript/source/evidence-body.tex` §174), dengan
...
- **entri 198 — Depth Anything 3** (2025, `lin2025depthanything3`, arXiv 2511.10647,
  ByteDance Seed; disitasi di `manuscript/source/evidence-body.tex` §133). Menerima **sembarang jumlah
...
- Pseudo-depth berasal dari RGB yang sama → *error*-nya berkorelasi. Ia prior
  struktural, bukan sensor independen (`manuscript/source/evidence-body.tex` §133).
...
**Rencana eksperimen** (resep `manuscript/source/evidence-body.tex` §271–275): (1) generate
...
Kode eksperimen tinggal di **`experiments/code/`** (di luar repo ini; repo
...
di [`experiments/EKSPERIMEN.md`](experiments/EKSPERIMEN.md).
...
- Format entri dan daftar status ada di kepala `experiments/EKSPERIMEN.md`.
- Catat juga **nama skrip** di `experiments/code/` yang menghasilkan angka
...
- Sitasi ke korpus lokal sebutkan nomor entri dan/atau seksi `manuscript/source/evidence-body.tex`
  agar pengguna bisa memverifikasi sendiri.
- Sebelum mengubah angka agregat (182, 202, 20), periksa dampaknya ke `literature/synthesis.md`,
