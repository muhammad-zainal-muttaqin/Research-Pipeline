---
source_id: 022
bibtex_key: carion2020detr
title: End-to-End Object Detection with Transformers
year: 2020
domain_theme: Fondasi RGB
verified_pdf: 22_DETR.pdf
char_count: 79389
---
...
then optimize object-specific (bounding box) losses.
    Let us denote by y the ground truth set of objects, and ŷ = {ŷi }N    i=1 the
...
for class prediction and a box loss defined later:
                               N h
                               X                                                         i
        LHungarian (y, ŷ) =         − log p̂σ̂(i) (ci ) + 1{ci 6=∅} Lbox (bi , b̂σ̂ (i)) ,   (2)
                               i=1
...
ities p̂σ̂(i) (ci ) instead of log-probabilities. This makes the class prediction term
...
Liou (·, ·) that is scale-invariant. Overall, our box loss is Lbox (bi , b̂σ(i) ) defined as
λiou Liou (bi , b̂σ(i) ) + λL1 ||bi − b̂σ(i) ||1 where λiou , λL1 ∈ R are hyperparameters.
...
map f ∈ RC×H×W . Typical values we use are C = 2048 and H, W = H        0 W0
                                                                       32 , 32 .
...
                                End-to-End Object Detection with Transformers                       11
     ...
     self-attention(430, 600)                                            self-attention(450, 830)
...
There are three components to the loss: classification loss, `1 bounding box
...
13. He, K., Girshick, R., Dollár, P.: Rethinking imagenet pre-training. In: ICCV (2019)
14. He, K., Gkioxari, G., Dollár, P., Girshick, R.B.: Mask R-CNN. In: ICCV (2017)
...
17. Hu, H., Gu, J., Zhang, Z., Dai, J., Wei, Y.: Relation networks for object detection.
    In: CVPR (2018)
18. Kirillov, A., Girshick, R., He, K., Dollár, P.: Panoptic feature pyramid networks.
    In: CVPR (2019)
...
22. Lin, T.Y., Dollár, P., Girshick, R., He, K., Hariharan, B., Belongie, S.: Feature
    pyramid networks for object detection. In: CVPR (2017)
23. Lin, T.Y., Goyal, P., Girshick, R.B., He, K., Dollár, P.: Focal loss for dense object
    detection. In: ICCV (2017)
24. Lin, T.Y., Maire, M., Belongie, S., Hays, J., Perona, P., Ramanan, D., Dollár, P.,
...
27. Lüscher, C., Beck, E., Irie, K., Kitza, M., Michel, W., Zeyer, A., Schlüter, R., Ney,
...
39. Rezatofighi, S.H., Kaskman, R., Motlagh, F.T., Shi, Q., Cremers, D., Leal-Taixé,
...
41. Romera-Paredes, B., Torr, P.H.S.: Recurrent instance segmentation. In: ECCV
    (2015)
42. Salvador, A., Bellver, M., Baradad, M., Marqués, F., Torres, J., Giró, X.: Recurrent
...
52. Zhang, S., Chi, C., Yao, Y., Lei, Z., Li, S.Z.: Bridging the gap between anchor-based
    ...
    (2019)
53. Zhou, X., Wang, D., Krähenbühl, P.: Objects as points. arXiv:1904.07850 (2019)
20      Carion et al.
...
sion d is a function with the following signature (using d0 = M
                                                              d
                                                                , and giving
...
connections, dropout and layer normalization. In other words, denoting X̃q =
                              ¯ (q) the concatenation of attention heads, we have
mh-attn(Xq , Xkv , T, L) and X̄

                 Xq0 = [attn(Xq , Xkv , T1 ); ...; attn(Xq , Xkv , TM )]            (5)
                 X̃q = layernorm Xq + dropout(LXq0 ) ,
                                                             
                                                                                    (6)

where [;] denotes concatenation on the channel axis.
                                                                                0
Single head An attention head with weight tensor T 0 ∈ R3×d ×d , denoted by
attn(Xq , Xkv , T 0 ), depends on additional positional encoding Pq ∈ Rd×Nq and
...
after adding the query and key positional encodings [7]:
                ...
                [Q; K; V ] = [T10 (Xq + Pq ); T20 (Xkv + Pkv ); T30 Xkv ]           (7)

where T 0 is the concatenation of T10 , T20 , T30 . The attention weights α are then
...
is the aggregation of valuesPNweighted     by attention weights: The i-th row is given
by attni (Xq , Xkv , T 0 ) = j=1
                              kv
                                 αi,j Vj .
...
in our loss, together with a `1 loss on b̂:

              Lbox (bσ(i) , b̂i ) = λiou Liou (bσ(i) , b̂i ) + λL1 ||bσ(i) − b̂i ||1 ,                     (9)

where λiou , λL1 ∈ R are hyperparameters and Liou (·) is the generalized IoU [38]:

                                          |bσ(i) ∩ b̂i |       |B(bσ(i) , b̂i ) \ bσ(i) ∪ b̂i |
                                                                                                 
          Liou (bσ(i) , b̂i ) = 1 −                        −                                          .   (10)
                                          |bσ(i) ∪ b̂i |              |B(bσ(i) , b̂i )|
...
are computed by min / max of the linear functions of bσ(i) and b̂i , which makes
the loss sufficiently well-behaved for stochastic gradients. B(bσ(i) , b̂i ) means the
largest box containing bσ(i) , b̂i (the areas involving B are also computed based
...
over Union. If we denote by m̂ the raw mask logits prediction of the model, and
...
ity O(d2 HW +d(HW )2 ): O(d0 d) is the cost of computing a single query/key/value
embeddings (and M d0 = d), while O(d0 (HW )2 ) is the cost of computing the at-
tention weights for one head. Other computations are negligible. In the decoder,
each self-attention is in O(d2 N +dN 2 ), and cross-attention between encoder and
...
Losses We use linear combination of `1 and GIoU losses for bounding box re-
...
the standard `1 loss for bounding box regression. We performed a grid search
