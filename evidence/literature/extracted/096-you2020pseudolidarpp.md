...
Published as a conference paper at ICLR 2020
                                         ...
                                         P SEUDO -L I DAR++:
                                         ACCURATE D EPTH FOR 3D O BJECT D ETECTION IN
                                         AUTONOMOUS D RIVING
                                          Yurong You∗1 , Yan Wang∗1 , Wei-Lun Chao∗2 , Divyansh Garg1 , Geoff Pleiss1 ,
                                          ...
                                          chao.209@osu.edu
arXiv:1906.06310v3 [cs.CV] 15 Feb 2020
                                         ...
                                         through inexpensive commodity cameras. How-         Figure 1: An illustration of our proposed depth esti-
                                         ever, in spite of recent dramatic progress in       mation and correction method. The green box is the
                                         stereo-based 3D object detection brought by         ground truth location of the car in the KITTI dataset.
                                         pseudo-LiDAR (Wang et al., 2019a), a signif-        The red points are obtained with a stereo disparity net-
                                         ...
                                         faraway objects (which we want to detect early      propagation (blue points) with a few (yellow) LiDAR
                                         to allow time for reaction). The trade-off be-      measurements the car is squarely inside the green box.
                                         tween affordability and safety creates an ethical   (One floor square is 1m×1m.)
                                         dilemma.
                                             ∗
                                            Equal contributions
                                             1
                                            The information is obtained from the automotive LiDAR market report: http://www.woodsidecap.
...
with those based on a 64-beam LiDAR but can potentially save 95% in cost.
    ...
    2
      The Ibeo Wide Angle Scanning (ScaLa) sensor with 4 beams costs $600 (USD). In this paper we simulate
...
then back-projected into a 3D point cloud: a pixel (u, v) will be transformed to (x, y, z) in 3D by
                     ...
                     z = Z(u, v),         x=                  ,         y=                 ,                    (1)
                                                     fU                           fV
where (cU , cV ) is the camera center and fU and fV are the horizontal and vertical focal length. The
...
the same 3D location. We can therefore derive the depth map Z via the following transform,
                                         fU × b
                           Z(u, v) =                 (fU : horizontal focal length).                            (2)
                                         D(u, v)
...
constructs Cdisp (u, v, d, :) by concatenating features of hl (u, v) and hr (u, v + d), followed by layers
   3
     Recently, Srivastava et al. (2019) proposed to lift 2D monocular images to 3D representations (e.g., bird’s-eye
...
obtained from LiDAR (yellow) and stereo (purple)             Figure 4: Depth estimation errors. We compare
corresponding to a car in KITTI, seen from the bird’s-       depth estimation error on 3,769 KITTI validation im-
eye view (BEV). Points from the disparity cost volume        ages, taking 64-beam LiDAR depths as ground truths.
are stretched out and noisy; while points from the           We separate pixels according to their true depths (z).
depth cost volume capture the car contour faithfully.        See the text and appendix for details.
...
3    S TEREO D EPTH N ETWORK (SDN)
A stereo network designed and learned to minimize the disparity                                   100
error (cf. Equation 4) may over-emphasize nearby objects with                                      80
                                                                                 depth (meters)
...
for a given error in disparity δD, the error in depth δZ increases                                 40
quadratically with depth:                                                                          20

                   1        1                                                                       00   20    40       60      80   100
             Z∝      ⇒ δZ ∝ 2 δD ⇒ δZ ∝ Z 2 δD.                            (5)                                disparity (pixels)
                   D       D                                                       Figure 2: The disparity-to-
The middle term is obtained by differentiating Z(D) w.r.t. D. In                   depth transform. We set fU =
particular, using the settings on the KITTI dataset (Geiger et al.,                721 (in pixels) and b = 0.54
2012; 2013), a single pixel error in disparity implies only a 0.1m                 (in meters) in Equation 2, which
...
Published as a conference paper at ICLR 2020
    ...
    Dense Pseudo-LiDAR Point Cloud                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            Depth
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              Correction

       Sparse LiDAR Point Cloud                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      Corrected Pseudo-LiDAR Point Cloud
...
We construct    the new  depth volume, Cdepth , based on the intuition that Cdepth (u, v, z, :) and
             fU × b
      
Cdisp u, v,          , : should lead to equivalent “cost”. To this end, we apply a bilinear interpolation
                z
...
of any point from the depths of its neighbors in Ni . We can solve for these weights with the following
constrained quadratic optimization problem:
             W = arg minW kZ − W Zk22 ,            s.t. W 1 = 1 and Wij = 0 if j ∈
                                                                                 / Ni .              (7)
Here 1 ∈ Rn+m denotes the all-ones vector. As long as we pick k > 3 and the points are in general
position there are infinitely many solutions that satisfy Z = W Z, and we pick the solution with the
minimum L2 norm (obtained with slight L2 regularization).
Depth Correction. Let us denote the corrected depth values as Z 0 ∈ Rn+m , with Z 0 = [ZL0 ; ZP0 L ]
...
W ; i.e. if point i : 1 ≤ i ≤
                            Pn is moved to its new depth Gi , then its neighbors in Ni must also be
corrected such that Gi ≈ j∈Ni Wij Zj0 . Further, the neighbors’ neighbors must be corrected and
the depth of the few n points propagates across the entire graph. We can solve for the final Z 0 directly
with another quadratic optimization:
                        Z 0 = arg minZ 0 kZ 0 − W Z 0 k2 ,            0
                                                                s.t. Z1:n = G.                       (8)
...
points similarly, i.e. Z 0 = Z + 1δ: as Z = W Z and W 1 = 1 we must have W (Z + 1δ) = Z + 1δ.
...
sparse 4-beam LiDAR. PL stands for PSEUDO -L I DAR. Our PSEUDO -L I DAR ++ (PL++) with enhanced depth
...
We refer to our combined method (SDN and GDC) for 3D object detection as PSEUDO -L I DAR++
...
We compare to four stereo-based detectors: PSEUDO -L I DAR (PL in short) (Wang et al., 2019a),
3DOP (Chen et al., 2015), S-RCNN (Li et al., 2019b), and MLF- STEREO (Xu & Chen, 2018).
Stereo depth network (SDN). We use PSMN ET (Chang & Chen, 2018) as the backbone for our
...
We also train a PSMN ET in the same way for comparison, which minimizes disparity error.
...
Table 2: Results on the car category on the test set.          Table 3: Ablation study on depth estimation. We
We compare PL++ (blue) and 64-beam LiDAR (gray),               report APBEV / AP3D (in %) of the car category at
using P-RCNN, and report APBEV / AP3D at IoU=0.7.              IoU= 0.7 on KITTI validation. DL: depth loss.
 Input signal        Easy     Moderate       Hard                Stereo depth       Easy        Moderate          Hard
 PL++ (SDN)       75.5 / 60.4 57.2 / 44.6 53.4 / 38.5            PSMN ET         73.4 / 62.3    56.0 / 44.9    52.7 / 41.6
 PL++ (SDN + GDC) 83.8 / 68.5 73.5 / 54.7 66.5 / 51.2            PSMN ET + DL    80.1 / 65.5    61.9 / 46.8    56.0 / 43.0
 LiDAR            89.5 / 85.9 85.7 / 75.8 79.1 / 68.3            SDN             82.0 / 67.9    64.0 / 50.1    57.3 / 45.3

Table 4: Ablation study on leveraging sparse Li-               Table 5: Results of pedestrians (top) and cy-
DAR. We report APBEV / AP3D (in %) of the car cate-            clists (bottom) on KITTI validation. We apply F-
gory at IoU= 0.7 on KITTI validation. L#: 4-beam               P OINT N ET Qi et al. (2018) and report APBEV / AP3D
LiDAR signal alone. SDN + L#: pseudo-LiDAR with                (in %) at IoU= 0.5, following Wang et al. (2019a).
depths of landmark pixels replaced by 4-beam LiDAR.
                                                                 Stereo depth      Easy        Moderate          Hard
The best result of each column is in bold font.                  PSMN ET        41.3 / 33.8    34.9 / 27.4    30.1 / 24.0
  Stereo depth      Easy       Moderate         Hard             SDN            48.7 / 40.9    40.4 / 32.9    34.9 / 28.8
  SDN            82.0 / 67.9   64.0 / 50.1   57.3 / 45.3         SDN + GDC      63.7 / 53.6    53.8 / 44.4    46.8 / 38.1
  L#             73.2 / 56.1   71.3 / 53.1   70.5 / 51.5         PSMN ET        47.6 / 41.3    29.9 / 25.2    27.0 / 24.9
  SDN + L#       86.3 / 72.0   73.0 / 56.1   67.4 / 54.1         SDN            49.3 / 44.6    30.4 / 28.7    28.6 / 26.4
  SDN + GDC      88.2 / 75.1   76.9 / 63.8   73.4 / 57.4         SDN + GDC      65.7 / 60.8    45.8 / 40.8    42.8 / 38.0
...
Depth loss and depth cost volume. To turn a disparity network (e.g., PSMN ET) into SDN, there
...
volume brings another 2 ∼ 3% gain4 .
   ...
   4
     We note that, the degree of improvement brought by the depth loss and depth cost volume depends on the
...
P OINT N ET (Qi et al., 2018) for detecting pedestrians and cyclists. Table 5 shows the results: our
methods significantly boosts the performance.

                          LiDAR                                     Pseudo-LiDAR

               Pseudo-LiDAR++ (SDN)                      Pseudo-LiDAR++ (SDN + GDC)
...
the right. It can be seen that the point clouds generated by PSEUDO -L I DAR ++ (SDN alone or SDN
...
+GDC) align better with LiDAR than that generated by PSEUDO -L I DAR (PSMN ET). For nearby
...
to the right), PSEUDO -L I DAR with depth estimated from PSMN ET predicts objects (red boxes) that
are deviated from the ground truths (green boxes). Moreover, the noisy PSMN ET points also leads to
false negatives. In contrast, the detected boxes by our PSEUDO -L I DAR ++, either with SDN alone or
...
PSEUDO -L I DAR ++ with fewer LiDAR beams, comparisons to depth completion methods, analysis
...
system, PSEUDO -L I DAR ++ (SDN + GDC), performs almost on par with 64-beam LiDAR systems
...
generous support by Zillow and SAP America Inc. We thank Gao Huang for helpful discussion.

R EFERENCES
Florian Chabot, Mohamed Chaouch, Jaonary Rabarisoa, Céline Teulière, and Thierry Chateau. Deep
...
Arsalan Mousavian, Dragomir Anguelov, John Flynn, and Jana Košecká. 3d bounding box estimation
...
Siddharth Srivastava, Frederic Jurie, and Gaurav Sharma. Learning 2d to 3d lifting for object detection
...
on a grid). We can then obtain features for each grid in Cdepth (i.e., Cdepth (u, v, z, :)) by bilinear
                                                                                         fU × b
                                                                                                
interpolation over features on grids of Cdisp around the non-grid location (i.e., u, v,           ). We
                                                                                             z
applied the “grid_sample” function in PyTorch for bilinear interpolation.
We use PSMN ET (Chang & Chen, 2018) as the backbone for our stereo depth estimation network
(SDN). The only change is to construct the depth cost volume before performing 3D convolutions.

B     G RAPH - BASED D EPTH C ORRECTION (GDC) A LGORITHM

Here we present the GDC algorithm in detail (see algorithm 1). The two steps described in the
...
switch the objective to minimizing the L2 -norm of W and set Z − W Z = 0 as a constraint5 . For the
...
Output: Corrected depth map Z 0 ∈ R(n+m)×1
function GDC(Z, P, G, K)
   Solve: W = arg minW ∈R(n+m)×(n+m) kW k2
          s.t. Z − W · Z = 0,
                W
                Pij = 0 if j ∈/ Ni (i.e., the set of neighbors of the ith point) according to P ,
                   j Wij = 1 for ∀i = 1, . . . , n + m.
...
C.1    S PARSE L I DAR GENERATION
...
point cloud from KITTI dataset in detail. For every point (xi , yi , zi ) ∈ R3 of the point cloud in one
   5
     These two problems yield identical solutions but we found the second one is easier to solve in practice.
...
bird’s-eye view on one example scene in Figure 7.
                ...
                (a) 2-beam                    (b) 4-beam                  (c) 64-beam (full)
...
D     A DDITIONAL R ESULTS , A NALYSES , AND D ISCUSSIONS
D.1    A BLATION STUDY
...
PSMN ET to PSMN ET +DL), constructing the depth cost volume (from PSMN ET +DL to SDN), and
...
D.2    U SING FEWER L I DAR BEAMS
...
IoU= 0.7 on the KITTI validation set. DL stands for depth loss.
      ...
      PSMN ET             73.9 / -    54.0 / - 46.9 / -        74.9 / 61.9 56.8 / 45.3 49.0 / 39.0
      PSMN ET + DL        75.8 / -    56.2 / - 51.9 / -        75.7 / 60.5 57.1 / 44.8 49.2 / 38.4
...
Table 9: Comparison of GDC and P N P for 3D object detection. We report APBEV / AP3D (in %) of the car
category at IoU= 0.7 on the KITTI validation set, using SDN + P N P or SDN + GDC for depth estimation and
P-RCNN or PIXOR? for detection.

                                P-RCNN                                     PIXOR?
        Input signal
                       Easy     Moderate       Hard       Easy             Moderate   Hard
        SDN + P N P 86.3 / 72.1 73.3 / 58.9 67.2 / 54.2 79.1 / -           64.2 / - 54.0 / -
        SDN + GDC 88.2 / 75.1 76.9 / 63.8 73.4 / 57.4 84.0 / -             71.0 / - 65.2 / -

D.3    D EPTH CORRECTION VS . DEPTH COMPLETION
...
is on fusing a much sparser 4-beam LiDAR and stereo depths. As such, the two problems are not
   6
     Torres-Mendez & Dudek (2004) use MRFs and may thus require less (or even no) training data compared to
...
         Published as a conference paper at ICLR 2020
                          ...
                          0.150
                                                                        GDC
median error in a scene   0.125                                         PnP
                          0.100
                          0.075
                          0.050
                          0.025
         ...
         Figure 8: Comparison of GDC and P N P for depth cor-                          Figure 9: Median depth estimation errors
         rection. We report the median of absolute errors on the                       w.r.t. the shortest distances to 4-beam LiDAR
         KITTI validation set. See text for details.                                   points on KITTI validation set.
        ...
        Here we empirically compare to P N P (Wang et al., 2018), a recently proposed depth completion
        algorithm compatible with any (even stereo) depth estimation network, similar to GDC. We use
        SDN for initial depth estimation, and evaluate GDC and P N P by randomly selecting a fraction of
        LiDAR points as provided ground truths and calculating the median absolute depth errors on the
        remaining LiDAR points. As shown in Figure 8, GDC outperforms P N P by a large margin. Table 9
        shows a further comparison to P N P on 3D object detection. We apply P N P and GDC respectively to
        ...
        validation set. In either case, SDN + GDC outperforms SDN + P N P by a notable margin.
        ...
        With the following optimizations for implementation,
         ...
         D.5                 S TEREO DEPTH VS . DETECTION
...
not on the 4-beam LiDAR. The improvement of SDN (+ GDC) over PSMN ET becomes larger as we
consider pixels farther away. Table 13 further demonstrates the relationship between depth quality
and detection accuracy: SDN (+ GDC) significantly outperforms PSMN ET for detecting faraway
...
each PSEUDO -L I DAR point and the 4-beam LiDAR points in Figure 9. We can see that the closer the
PSEUDO -L I DAR points are to the 4-beam LiDAR points, the bigger improvement GDC can bring.

D.6    C ONNECTED C OMPONENTS IN KNN GRAPHS OF PSEUDO -L I DAR POINTS BY SDN

Here, we provide empirical analysis on the relationship between the k we choose in building the K-
nearest-neighbor graph of PSEUDO -L I DAR points by SDN and the number of connected components
...
Figure 10: IoU vs. APBEV on KITTI validation set        Figure 11: k vs. average number of connected
on the car category (moderate).                         components in KNN graphs of PSEUDO -L I DAR
                                                        points by SDN.
...
last two rows we show the number of car objects in KITTI object train and validation sets within different ranges.
                     ...
                     PSMN ET                   65.6 / 54.0     15.8 / 6.9       0.0 / 0.0
                     SDN                       68.6 / 56.7     27.4 / 11.3      0.7 / 0.0
                     SDN + GDC                 84.7 / 67.8     49.9 / 31.5      2.5 / 1.0
                      L I DAR                  88.5 / 84.0     69.9 / 51.5      8.9 / 3.4
                     # O BJECTS -T RAIN           6903            3768             76
                     # O BJECTS -VAL              7379            3542             39

D.8    Q UALITATIVE RESULTS
...
the point clouds generated by PSEUDO -L I DAR ++ (SDN alone or SDN +GDC) align better with
LiDAR than those generated by PSEUDO -L I DAR (PSMN ET). For nearby objects (i.e., bounding
...
PSEUDO -L I DAR with depth estimated from PSMN ET predicts objects (red boxes) deviated from the
ground truths (green boxes). Moreover, the noisy PSMN ET points also leads to several false positives
or negatives. In contrast, the detected boxes by our PSEUDO -L I DAR ++, either with SDN alone or
with SDN +GDC, align pretty well with the ground truth boxes, justifying our targeted improvement
in estimating faraway depths. In Figure 12, we see one failure case for both PSEUDO -L I DAR
and PSEUDO -L I DAR ++: the most faraway car is missed, while LiDAR signal can still detect it,
